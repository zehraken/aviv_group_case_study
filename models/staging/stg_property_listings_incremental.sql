...
--we can select max date, or we can select the last 7 days, or we can select the last 30 days,
--it depends on the business requirements and the data volume or data update period. 
--If the data volume is small, 
--we can select the last 30 days, if the data volume is large, we can select the last 7 days, 
--if the data volume is very large, we can select the max date.
{% if is_incremental() %}
    where pl.updated_at > (select max(updated_at) from {{ this }})
{% endif %}